...
async def add_documents_to_supabase(
    ...
    for i in range(0, len(documents), batch_size):
        ...
        try:
            ...
            print(
                f"Successfully inserted {successful_insertions}/{len(batch)} documents individually in batch {i//batch_size + 1}"
            )
...
def update_source_info(client: Client, source_id: str, summary: str, word_count: int):
    ...
    try:
        ...
        result = (
            ...
            .update({"summary": summary, "total_word_count": word_count, "updated_at": "now()"})
        ...
        if not result.data:
            client.table("sources").insert(
                {"source_id": source_id, "summary": summary, "total_word_count": word_count}
            ).execute()
